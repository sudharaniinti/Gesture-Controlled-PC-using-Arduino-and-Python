...
const int trigger1 = 2; //Trigger pin of 1st Sensor (LEFT SENSOR)
const int echo1 = 3; //Echo pin of 1st Sensor
const int trigger2 = 4; //Trigger pin of 2nd Sensor (RIGHT SENSOR)
...
void loop() 
{
  ...
  if (distance_L>=10&& distance_L<=20)
  {
    ...
    if (distance_L>=10&& distance_L<=20)
    {
      ...
      {
        calculate_distance(trigger1,echo1);
        distance_L=dist;if (distance_L<10) //Hand pushed in
        {
          Serial.println ("Vup"); 
          delay (300);
        }
